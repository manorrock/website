...
- **`/speckit.checklist`** can run any time after `specify`. It generates a custom quality checklist for *human* consumption — validating the requirements for completeness, clarity, and consistency, in the project's own framing, "unit tests for English." The quickstart slots it in after `plan` as a convenient moment to review, but it is not gated on the plan: run it whenever you want a readable check on where the requirements stand.
...
But I want to be careful not to oversell "core" as "only." This sequence is *the* process that defines what spec-driven development means in Spec Kit — it is the reference process, the one the whole toolkit is named for. It is not, however, the only process the machinery can express. The same primitives that customize this sequence can also drive entirely different processes: **workflows** chain commands, prompts, shell steps, and human gates into a single automated run — with conditional logic, loops, and fan-out/fan-in — and **workflow steps** are the typed units those runs are built from. Spec Kit even ships the core sequence itself as a runnable `speckit` workflow, and you can author your own to encode a process that is nothing like the built-in one. The core process is the center of gravity, not the boundary — which means the machinery I spent four posts on unlocks more than the single sequence it was easiest to demonstrate on.
